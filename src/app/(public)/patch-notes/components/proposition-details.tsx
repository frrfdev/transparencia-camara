...
export const PropositionDetails = ({ proposition }: PropositionDetailsProps) => {
  const { data: propositionDetails } = useGetPropositionDetailsQuery({ propositionId: proposition.id });
  const { data: propositionAuthors } = useGetPropositionAuthorsQuery({ propositionId: proposition.id });

  console.log(propositionDetails);
        ...
        <table className="drop-shadow-md">
          <tbody>
            <tr className="border-b-2 border-gray-400/50">
              ...
              <td className="bg-white text-black p-2 w-1/2">{propositionDetails?.siglaTipo}</td>
            </tr>
            <tr className="border-b-2 border-gray-400/50">
